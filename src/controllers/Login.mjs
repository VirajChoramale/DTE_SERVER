...
const login = async (req, res, next) => {
  ...
  if (resp.code == 200) {
    ...
    let sms_resp = null;
    const resp_arr = {};
    await sendOtpSMS(resp.data[0].mobile, resp.data[0].username)
      .then((res) => {
    ...
    res.status(200).send({ msg: resp_arr.msg, res: resp.res, lid: token });
    next();
  } else {
    res.json(resp);
  }
...
const verify_otp = async (req, res) => {
  const tes = await executeReadQuery(
    `select latest_otp,password,role from users_new where username='${req.user.uname}'`
  );
  if (tes[0].latest_otp == req.body.otp) {
    const token = jsonwebtoken.sign(
      { uname: req.user.uname, role: tes[0].role, cred: tes[0].password },
      process.env.HMAC
    );
