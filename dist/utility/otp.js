...
export const genrateRandomText = async length => {
  const characters = "ABCDEFG+HIJKL#MNOPQR%=STUVWXYZab*cd=!efghijklmnopqrstuvwxyz0123456789";
  let result = "";
...
function generateKey(message) {
  return crypto.createHash("sha512").update(`${username}${senderid}${message}${deptSecureKey}`).digest("hex");
}
