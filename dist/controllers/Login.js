...
const login = async (req, res, next) => {
  ...
  if (resp.code == 200) {
    ...
    let sms_resp = null;
    const resp_arr = {};
    await sendOtpSMS(resp.data[0].mobile, resp.data[0].username, resp.data[0].email, resp.data[0].name).then(res => {
...
const verify_otp = async (req, res) => {
  ...
  if (tes[0].latest_otp === req.body.otp) {
    ...
    await update_table("users_new", "username", req.user.uname, ["latest_otp"], [""]);
    res.setHeader("Authorization", `Bearer ${token}`);
...
export const resetPassword = async (req, res) => {
  ...
  if (user[0]) {
    ...
    let sms_resp = null;
    const resp_arr = {};
    await sendOtpSMS(user[0].mobile, user[0].username, user[0].email, user[0].name).then(res => {
      const resp_arr = res.split(",");
      sms_resp = resp_arr[0];
    }).catch(e => console.log("error in sms api" + e));
    if (sms_resp === "402") {
